...
```{tex}
En las tablas resultantes se puede apreciar un resumen estadistico de cada elemento, agrupado de acuerdo a el tipo de cancer y el nivel de expresion de cada gen. 
Se puede visualizar> varibale, tipo, numero de datos o muestras, la media, la moda , el rango, ademas de las desviacion tipica y el error estandar. Si analizaramos a profundidad cada tabla podriamos comprender la forma en la que se comportan estos datos.
```
...
```{tex}
En la figura del boxplot se puede observar el nivel del expresion para el tipo de cancer con codigo KIPR o sea el Carcinoma renal de células papilares. S epuede observar que la expresion oscila entre valores de -1 y -3 aproximadamente por lo cual seria factible decir que la expresion de este gen no se asocia a este tipo de cancer, pero tal vez si su silenciamiento, lo cual yo interpreto como la subexpresion. 
```
